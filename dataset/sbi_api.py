...
class SBI_API:
	...
	def __call__(self,img,landmark=None,sri_image=None):
		try:  
			...
			# apply sbi
			img_r, img_f, mask_f = self.self_blending(img.copy(), landmark.copy())
			sri_r, sri_f, sri_mask_f = self.self_blending(sri_image.copy(), landmark.copy())  # Apply self-blending to sri_image

			if self.phase == 'train':
				transformed = self.transforms(
					image=img_f.astype('uint8'),
					image1=img_r.astype('uint8'),
					image2=sri_r.astype('uint8'),
					image3=sri_f.astype('uint8'),  # Add sri_f_image as an additional target
				)
				img_f = transformed['image']
				img_r = transformed['image1']
				sri_image = transformed['image2']
				sri_f_image = transformed['image3']  # Update sri_f_image with the transformed image

			return img_f, img_r, sri_image, sri_f_image  # Return sri_image, sri_f_image along with img_f and img_r
	...
	def get_transforms(self):
		return alb.Compose([
			...
			additional_targets={'image1': 'image', 'image2': 'image', 'image3': 'image'},  # Add 'image3' as an additional target
		p=1.)
...
def create_bbox_mask(image, landmarks, margin=0):
    ...
    # Reduce margin if the bounding box covers the entire image
    while (min_x == 0) and (min_y == 0) and (max_x == image.shape[1]) and (max_y == image.shape[0]) and (margin > 0):
        margin -= 5
        min_x = max(0, min_x - margin)
        min_y = max(0, min_y - margin)
        max_x = min(image.shape[1], max_x + margin)
